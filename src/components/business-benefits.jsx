...
export function BusinessBenefits() {
  return (
    <section aria-label="Business benefits" className="py-16 sm:py-20 border-y border-ink-700">
      <div className="max-w-5xl mx-auto px-6">
        <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-ink-700">
          {BENEFITS.map((text, i) => (
            <p
              key={i}
              className="text-center text-sm sm:text-base text-text leading-relaxed px-0 sm:px-8 py-6 sm:py-0"
            >
              {text}
            </p>
          ))}
