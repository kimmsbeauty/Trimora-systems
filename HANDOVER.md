...
| # | Item | Status |
|---|---|---|
| 1 | Lead notification -- **`notify-new-lead`** (alerts Lucy by email + WhatsApp when a lead comes in) | ✅ Infrastructure built. ⬜ **CONFIRMED NOT ACTIVE (2026-07-06)** -- `RESEND_API_KEY` doesn't exist in the project's Edge Function Secrets at all (verified via Ctrl+F search on the Supabase dashboard, 0/0 matches), so email is a guaranteed no-op. `NOTIFY_EMAIL_TO` and the Africa's Talking keys are also absent, so WhatsApp is unconfirmed/likely also inactive. See Item 3 below -- this shares the exact same root cause and fix as `send-lead-confirmation`. |
...
| 3 | Qualification funnel + Cal.com booking | ✅ Qualification funnel and the Cal.com embed itself confirmed working (`NEXT_PUBLIC_CAL_LINK` = `trimorapos-vp9pyt/trimora-test`, real test booking completed). ⬜ **`send-lead-confirmation` (alerts the LEAD) is CONFIRMED NOT WORKING (2026-07-06)** -- this was wrongly marked working earlier the same day based on a false positive: the "confirmation email" received was Cal.com's own native booking confirmation (triggered by entering a real email at the *booking* step), not the Trimora-branded one from this function. `RESEND_API_KEY` does not exist anywhere in the project's Edge Function Secrets (confirmed via literal Ctrl+F search on the dashboard, 0/0 matches) -- so the function has been silently no-op'ing the email step every single time, exactly as its own code predicts when the key is missing. **Root cause and fix are the same as Item 1, below -- get a Resend API key and add it as a Supabase secret.** Lesson for future sessions: a "test" that used a fake/throwaway email address at the lead-form step doesn't actually test `send-lead-confirmation`, since it can't email an address that doesn't exist -- only a real email at the initial contact step proves anything. Debugging trail retained since it's still useful: found and fixed a 2-day-old typo in Vercel's env var name (`NEXT_PUBLIC_SUPABASE_UR`, missing the L) that had been separately breaking the entire lead form; discovered Supabase's dashboard Invocations tab does NOT capture response bodies, only status/headers -- added explicit `console.log(result)` to both `send-lead-confirmation` (commit `5912584`) and `notify-new-lead` for when this gets retested properly. **Remaining cosmetic cleanup, not blocking:** the Cal.com event itself is still named "trimora test" and the account's display name has a typo ("Trimora Sysytems") -- both fixable in Cal.com's own dashboard, not code. |
